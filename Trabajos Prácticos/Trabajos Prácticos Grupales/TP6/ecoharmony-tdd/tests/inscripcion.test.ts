...
describe("Inscripción a actividad", () => {
  ...
  it("prueba 4 falla: no hay turno a las 18:00", async () => {
    const { servicio } = setupTest();
    const hora = "18:00";
  ...
  it("prueba 4 pasa: último turno 17:00", async () => {
    const { servicio, mailer } = setupTest();
    expect(turnoDentroHorarioActividad("17:00")).toBe(true);
    await servicio.inscribir(
      solicitudOk({
        turno: { actividad: "Safari", fecha: "2026-05-27", hora: "17:00" },
      })
    );
    expect(mailer.envios).toHaveLength(1);
  });
  ...
  it("regla: menos de 24 hs antes del turno", async () => {
    const { servicio } = setupTest();
    await expect(
      servicio.inscribir(
        solicitudOk({
          turno: { actividad: "Safari", fecha: "2026-05-20", hora: "10:00" },
        })
      )
    ).rejects.toThrow(AnticipacionInvalidaError);
  });

  it("regla: 24 hs antes del turno alcanza", async () => {
    const { servicio, mailer } = setupTest();
    await servicio.inscribir(
      solicitudOk({
